...
from .tokenization_utils import _is_whitespace, _is_control
from .tokenization_utils import convert_by_vocab, printable_text
...
class FullTokenizerSpm(PreTrainedFullTokenizer):
    ...
    def covert_token_to_id(self, token):
        ...
        """
        if self.sp_model:
            return self.sp_model.PieceToId(printable_text(token))
        else:
